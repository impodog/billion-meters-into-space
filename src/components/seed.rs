...
impl Seed {
    ...
    pub fn rand_center(&mut self) -> Vec2 {
        let x = self.rand_range(-WIDTH / NON_CENTER..WIDTH / NON_CENTER);
        let y = self.rand_range(-HEIGHT / NON_CENTER..HEIGHT / NON_CENTER);
        Vec2::new(x, y)
    }

    pub fn rand_center_vec3(&mut self) -> Vec3 {
        let result = self.rand_center();
        Vec3::new(result.x, result.y, 0.0)
    }
